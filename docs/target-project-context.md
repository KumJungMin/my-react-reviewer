...
리뷰 품질을 높이려면 대상 프로젝트가 짧은 지식 파일을 제공하는 것이 좋다. 목적은 모델에 전체 문서를 먹이는 것이 아니라, diff만으로 알 수 없는 제약을 작게 전달하는 것이다.
...
repository context 전체는 `MAX_CONTEXT_CHARS`로 제한된다. 기본값은 `30000`이다.

GitHub repository variable로 조정할 수 있다.
...
- 작은 프로젝트: `15000`~`25000`
- 일반적인 React 앱: `30000`
- architecture 문서가 많은 프로젝트: `40000`~`60000`

너무 크게 올리면 리뷰 비용이 늘고, 오래된 문서가 판단을 흐릴 수 있다.
