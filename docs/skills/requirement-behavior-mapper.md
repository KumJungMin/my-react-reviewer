...
- 요구사항 요약
- 누락/위험 고려사항
- Codex 스킬이 이어받기 쉬운 구조화 handoff 블록(요청 시)
- 다음 스킬에 바로 붙여 넣을 수 있는 요청 템플릿(요청 시)
- 구현 전 확인 및 답변
```
...
6. 유저가 이해하기 쉬운 말로 시나리오, 동작 단위, 요구사항 커버리지를 정리합니다.
7. 구현 slice는 `구현 단위`, `관련 동작`, `구현할 내용`, `나누는 이유`처럼 유저 친화적인 한글 표로 제안하고, 추천 커밋 경계는 `Codex Skill Handoff` 안에 포함합니다.
8. `business-feature-builder`나 `react-workflow-orchestrator`가 바로 이어받기 쉬운 `Codex Skill Handoff` 블록을 제공합니다.
9. 사용자가 요청했거나 구현으로 이어가려는 경우에만 다음 스킬에 바로 붙여 넣을 수 있는 요청 템플릿을 제공합니다.
10. 구현 전 질문과 사용자가 바로 채울 수 있는 답변 템플릿을 `구현 전 확인 및 답변` 섹션 하나로 합칩니다. 질문에 답할 때는 "위 질문에 대한 답변을 반영해서 Codex Skill Handoff를 만들어줘", 답변하지 않고 진행할 때는 "구현 요청 템플릿을 만들어줘"를 함께 제시합니다.
11. 다음 단계가 `business-feature-builder` 구현인지, 제품 질문 정리인지, scope 축소인지 제안합니다.
...
- `skills/requirement-behavior-mapper/references/next-skill-request-template.md`
