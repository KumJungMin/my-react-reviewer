...
description: "Use when the user wants to review or improve React, TSX, JSX, hooks, component tests, or nearby frontend code quality in Codex or VS Code Codex. Focuses on React correctness, hook usage, state management, maintainability, accessibility, performance, and testability. Delegates broad project/team convention checks to team-convention-checker. Supports review-only, apply-selected-review-items, and direct-fix modes."
---
...
All user-facing responses, questions, summaries, artifact prose, section headings, and guidance must be written in Korean by default. Keep code identifiers, file paths, commands, API names, reviewer ids, and quoted source names in their original form. If another instruction names an English response section, translate that section heading into natural Korean when presenting it to the user.

This skill focuses on code quality. It does not perform a broad team-convention audit.

Use `team-convention-checker` instead when the main request is about folder structure, file placement, export style, naming policy, design-system usage rules, state-management conventions, or test location. Existing naming and layering reviewer lenses may still be used when those issues create concrete code-quality risk or the user explicitly asks for that review lens.

## Customization

Before applying default review priorities, read project-level review overrides when they exist:

- `.codex/react-feature-workflow/react-review-policy.md`
- `.react-ai-reviewer/review-policy.md`
- `.react-ai-reviewer/context.md`
- `docs/review-policy.md`
- `AGENTS.md`

Use project overrides for review priorities, allowed reviewer lenses, severity mapping, noise-control rules, validation commands, and output expectations.

Do not use review policy overrides for broad team convention checks. Route those to `team-convention-checker`.
