...
- `category`는 주로 `react`, `hooks`, `bug` 중에서 고른다.
- `reason`에는 어떤 timing assumption, stale synchronization, source-of-truth 혼합이 있는지 적는다.
- `suggestion`은 data flow 단순화, cleanup 보강, source-of-truth 정리, imperative 경계 축소 방향으로 쓴다.
