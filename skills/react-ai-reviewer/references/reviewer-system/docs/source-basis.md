...
이 프로젝트는 React 코드 리뷰를 하나의 거대한 프롬프트로 처리하지 않고, 권위 있는 자료별 reviewer 문서를 별도 렌즈로 유지한 뒤 final reviewer가 최종 우선순위를 정리하도록 설계했다.
...
| `maintainability` | 수정 용이성, 응집도, 단일 책임, 추상화 레벨 일관성 검토 |
