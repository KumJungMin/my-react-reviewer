...
모든 React 작업 스킬은 `react-workflow-orchestrator/references/shared-work-principles.md`의 공통 원칙을 공유합니다. 비사소한 코드 변경은 먼저 설계와 구현 리스트를 제시해 확인받고, 작업/커밋은 약 300 changed lines 내외의 reviewable batch로 나누며, 커밋 메시지는 타입을 필수로 포함하고 요약/본문은 한국어로 작성합니다.
...
- 타입이 있는 한국어 커밋 메시지와 `목적`, `방향`, `검증` 본문을 남기고 싶을 때
...
조건:
- 먼저 구현 리스트를 보여줘
- 비사소한 수정은 설계 확인 후 진행해줘
- 커밋은 300 changed lines 내외로 나눠줘
- 각 커밋 메시지는 `<type>: <한글 요약>` 형식으로 쓰고 본문에는 `목적`, `방향`, `검증`을 한국어로 적어줘
...
- 커밋 크기와 예외 기준은 `shared-work-principles.md`를 따르고, 초과 시 이유를 커밋 본문에 남긴다
- 커밋 메시지는 타입을 필수로 포함하고 본문에는 `목적`, `방향`, `검증`을 한국어로 남긴다
...
`requirement-behavior-mapper`가 만든 `codex_handoff`가 있으면 아래처럼 붙여서 요청할 수 있습니다.
...
codex_handoff:
  ...
  implementation_slices:
    - id: S1
      behavior_units: [B1]
      purpose: "[구현 목적]"
      dependencies: ["[API/usecase/state 등]"]
      validation: ["[검증할 동작]"]
  blocking_questions: []
  recommended_commit_boundaries:
    - id: C1
...
대상 프로젝트에 이미 설치된 `typescript` 패키지를 사용합니다. 패키지를 설치하거나 외부 scanner를 호출하지 않습니다. Markdown 출력에는 후보 신호 주변 source window가 포함됩니다.
...
```bash
node skills/react-ai-reviewer/scripts/analyze-react-ast.mjs --repo . --target src/features/payment/components/PaymentForm.tsx
```
...
1. `react-workflow-orchestrator`로 refactor plan, 동작 보존 범위, 커밋 단위, 검증 계획을 먼저 합의
2. `react-upgrade-workflow`가 상태/effect/책임 분리/테스트 가능성 개선을 담당
3. 필요하면 `react-ai-reviewer`의 `react-quality-lens` 기준으로 최종 리뷰

디자인 시스템 작업인 경우:

1. `react-workflow-orchestrator`로 design source, public API 방향, 커밋 단위, 검증 계획을 먼저 합의
2. `create-component-from-figma`로 초기 UI를 코드화
3. `gds-generator`로 public API, accessibility, slot contract, docs/examples, tests를 강화
4. 필요하면 `react-ai-reviewer`로 최종 코드 리뷰
